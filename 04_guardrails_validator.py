...
class PIIDetector(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        # print(f"DEBUG: Validating PII for: {value[:30]}...") # No emojis
        redacted_text = value
        found_pii = []
        ...
        if found_pii:
            print(f"  Found {len(found_pii)} PII items. Redacting...")
...
class JSONFormatter(Validator):
    ...
    def validate(self, value: str, metadata: dict):
        # print(f"DEBUG: Validating JSON for: {value[:30]}...")
        # Try direct parse
        try:
        ...
        try:
            ...
            return FailResult(error_message="Malformed JSON repaired", fix_value=json.dumps(parsed, indent=2))
        except json.JSONDecodeError as e:
            # Fallback error JSON if repair fails
            error_json = json.dumps({"error": "Unrecoverable JSON", "raw": value}, indent=2)
            return FailResult(error_message=f"Invalid JSON: {e}", fix_value=error_json)
...
def demo_json_guard():
    ...
    for label, text in test_cases:
        ...
        print(f"  Output: {str(result.validated_output)[:100]}")
